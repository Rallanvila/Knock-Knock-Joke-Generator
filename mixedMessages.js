...
const homeOwner = ["Who's There!?????", "Well who's there now?", "Hello...Well, I said Hello! Who's There??", "Hangon, I'm getting dressed...umm Who's there?", "I have a gun!!! Who is it!!!??", "Who in the Sam Hell is at my door!!!!"];
...
const randomGen = () => {
    ...
    let knockResp = [knocker[random]];
    let homeOwnerAnswer = homeOwner[Math.floor(Math.random() * 6)]
    let jokeResp = [joke[random]];
    //return knockResp + ' ' + knockResp + ' who!!???' + ' ' + jokeResp
    return `${knockKnock} ${homeOwnerAnswer} ${knockResp}. ${knockResp} who!!??? ${jokeResp}`
}
...
//     return jokeResponse
//console.log(knockKnock + ' ' + homeOwner + ' ' + knockGen(knocker) + ' ' + ' ' + jokeGen(joke));
